...
func (w *WorkflowService) ExecuteWorkflow(gtin string, productGroup string, quantity int, businessPlaceId int, expirationDays int) (*models.WorkflowResponse, error) {
	...
	seriesNumber := ""
	if productGroup == "pharma" {
		seriesNumber = fmt.Sprintf("SERIES-%d", time.Now().UnixNano()%1000000)
		logger.GetLogger().Infof("WORKFLOW: 📌 Добавлена серия для pharma: %s", seriesNumber)
	}
